...
S1AudioProcessorEditor::S1AudioProcessorEditor (S1AudioProcessor& p)
    : AudioProcessorEditor (&p), audioProcessor (p), noiseGenerator (p), mixerGUI (p), presetSave(p), presetsBrowser(p), programSelector(p), synthEngine1(p, 1), synthEngine2(p, 2)
{
    ...
    //addAndMakeVisible(&noiseGenerator);
